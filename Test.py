...
def load_text(self, whatever=None, filename=None):
    if not filename:
...
def test_dialog(event):
    ...
    btnTest.bind('<Button-1>', testing_click(libEntryT1, libEntryT2, libEntryT3))


def testing_click(libEntryT1, libEntryT2, libEntryT3):
    tests_count = int(libEntryT1.get())
    text_length = int(libEntryT2.get())
    dict_path = libEntryT3.get()

    # разобраться со словарем
    dict_digital = load_dict(dict_path)[0]
